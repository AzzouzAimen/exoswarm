...
structured failures, durable traces, viewer/agent isolation, cached reproducibility, clear setup, and
...
contracts constrain it; an append-only ledger and trace make the result auditable; and an explicit
viewer/agent boundary prevents visible reference answers from influencing the investigation.
...
| Originality & Problem Solving (10) | Innovation & Approach (10) | budgeted evidence selection, independent Critic review, append-only evidence, opaque agent targets, and viewer/agent isolation as one coherent trustworthy-agent pattern |
...
| Pitch, Demo & Documentation (20) | 3-Minute Video Demo (10) | concise live execution plus a legible architecture explanation; show evidence changing the path, agent/tool boundaries, Critic verdict, telemetry, and automatic comparison payoff |
...
- opaque agent identity and separate viewer-only catalog mapping,
- canonical result serialization and SHA-256 audit artifacts for reproduction,
...
- two visibly different demo trajectories and an immediate, understandable catalog comparison.
...
4. Complete one planet-like path and one eclipsing-binary-like path end to end through evidence,
   disposition and automatic viewer comparison. Make the paths visibly different.
...
6. Wire the mission-control UI to real state and events, including adaptive action cost, remaining
   budget, Critic verdict, failures, inference summary, and the final match verdict.
...
1. polished mission-control storytelling for evidence-driven branch differences, Critic review,
   cost allocation, failures, inference statistics, and viewer/agent isolation,
...
- viewer catalog truth is available to the human from the start but inaccessible from every agent context, tool, run state, and event,
